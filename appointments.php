...
*/
	...
	function pre_confirmation() {
		...
		$service = '<label><span>'. __('Examination: ', 'appointments' ).  '</span>'. apply_filters( 'app_confirmation_service', stripslashes( $service_obj->name ), $service_obj->name ) . '</label>';
		...
		if ( $price > 0 )
			$price = '<label><span>'.__('Price estimate: ', 'appointments' ).  '</span>'. apply_filters( 'app_confirmation_price', $price . " " . $this->options["currency"], $price ) . '</label>';
		else
			$price = 0;
	...
	function post_confirmation() {
		...
		if ( isset( $this->options["payment_required"] ) && 'yes' == $this->options["payment_required"] ) {
			die( json_encode(
							array(
							"cell"				=> $_POST["value"],
							"app_id"			=> encrypt_decrypt("encrypt", $insert_id),
							"refresh"			=> 0,
							"price"				=> $paypal_price,
	...
	function json_die( $field_name ) {
		die( json_encode( array("error"=>sprintf( __( 'Please provide %s', 'appointments'), $this->get_field_name($field_name)))));
	}
	...
	function get_monthly_calendar( $timestamp=false, $class='', $long, $widget ) {
		...
		$script  = '';
		$script .= '$(".app_monthly_schedule_wrapper table td.free").click(function(){$(".app_monthly_schedule_wrapper table .active").removeClass("active");$(this).addClass("active");';
...
*/
	...
	function load_scripts_styles( ) {
		wp_enqueue_script( 'printthis', $this->plugin_url . '/js/printthis.js', array('jquery'), $this->version );
		wp_enqueue_script( 'jquery-modal', $this->plugin_url . '/js/jquery.modal.min.js', array('jquery'), $this->version );
		wp_enqueue_script( 'jquery-validate', $this->plugin_url . '/js/jquery.validate.min.js', array('jquery'), $this->version );
		wp_enqueue_script( 'jquery-tooltipster', $this->plugin_url . '/js/jquery.tooltipster.min.js', array('jquery'), $this->version );
	...
	//	wp_enqueue_script( 'jquery-validate', $this->plugin_url . '/js/jquery.validate.min.js', array('jquery'), $this->version );
		...
		if ( !current_theme_supports( 'appointments_style' ) ) {
			...
			wp_enqueue_style( "modal", $this->plugin_url. "/css/jquery.modal.css", array(), $this->version );
			wp_enqueue_style( "tooltipster", $this->plugin_url. "/css/tooltipster.css", array(), $this->version );
			wp_enqueue_style( "tooltipster-light", $this->plugin_url. "/css/themes/tooltipster-light.css", array(), $this->version );
