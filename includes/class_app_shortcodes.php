...
class App_Shortcode_MyAppointments extends App_Shortcode {
	public function __construct () {
		$this->_defaults = array(
			...
			'provider_id' => array(
				'value' => 0,
				'help' => __('Enter the user ID of the provider whose list will be displayed. If omitted, current service provider will be displayed. Default: "0" (current service provider)', 'appointments'),
				'example' => '12',
			),
			'title' => array(
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		'<th class="my-appointments-name">'. __('Name', 'appointments' )
		. '</th><th class="my-appointments-service">'. __('Service', 'appointments' )
		. '</th>'  //<th class="my-appointments-worker">' . $provider_or_client
		. '<th class="my-appointments-date">' . __('Date/time', 'appointments' )
		. '</th><th class="my-appointments-status">' . __('Status', 'appointments' )
		. '</th><th class="my-appointments-print">' . __('Print', 'appointments' ) . '</th>');
		$colspan = 4;
...
*/
				...
				$ret .= '<td>';
				$ret .= '<a target="_blank" href="/print-form/?output=pdf&appointment_id=' .  encrypt_decrypt("encrypt", $r->ID) . '"><img src="/wp-content/themes/manawatu/images/print.png" /></a>';
				$ret .= '</td>';
		...
		if ( $_tablesorter && file_exists( $appointments->plugin_dir . '/js/jquery.tablesorter.min.js' ) )
			$appointments->add2footer( '
				...
				$(".app-my-appointments-cancel").change( function() {
					if ( $(this).is(":checked") ) {
						var cancel_box = $(this);
						if ( !confirm("'. esc_js( __("Are you sure you wish to cancel the selected appointment?","appointments") ) .'") ) {
...
class App_Shortcode_Services extends App_Shortcode {
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		$s .= '<div id="problem" class="modal">';
		$s .= '<h2>There was a problem!</h2>';
		$s .= '<p class="info"</p>';
		$s .= '<div class="gap"></div>';
		$s .= '<div class="gap"></div>';
		$s .= '<a href="#close" rel="modal:close" class="button-cancel">OK</a>';
		$s .= '</div>';
		
		$s .= '<div id="confirmed" class="modal">';
		$s .= '<h2>Appointment Confirmed</h2>';
		$s .= '<p class="info"></p>';
		$s .= '<div class="gap"></div>';
		$s .= '<p class="service"></p>';
		$s .= '<p class="datetime"></p>';
		$s .= '<p class="length"></p>';
		$s .= '<p class="name"></p>';
		$s .= '<p class="email"></p>';
		$s .= '<p class="phone"></p>';
		...
		$s .= '<a target="_blank" class="button-primary print">Print a copy</a>';
		$s .= '<a href="#close" rel="modal:close" class="button-cancel close">OK</a>';
		$s .= '</div>';
...
class App_Shortcode_Paypal extends App_Shortcode {
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		if(empty($appointments->options['currency']))
			$appointments->options['currency'] = 'NZD';
...
class App_Shortcode_Confirmation extends App_Shortcode {
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		$ret = '';
		$ret .= '<div class="appointments-confirmation-wrapper"><form id="appointmentForm" autocomplete="off"><fieldset>';
		$ret .= '<legend>';
		$ret .= $title;
		$ret .= '</legend>';
		...
		$ret .= '<label for="' . esc_attr(apply_filters('app-shortcode-confirmation-name_field_id', 'appointments-field-customer_name')) .'"><span>'. $name . ($appointments->options["ask_name"] ? ' *' : '') . '</span></label><input type="text" autocomplete="off" name="'. esc_attr(apply_filters('app-shortcode-confirmation-name_field_id', 'appointments-field-customer_name')) . '" class="appointments-name-field-entry" id="' . esc_attr(apply_filters('app-shortcode-confirmation-name_field_id', 'appointments-field-customer_name')) . '" />';
		$ret .= '</div>';
		
		$ret .= '<div class="appointments-email-field" style="display:none">';
		$ret .= '<label for="' . esc_attr(apply_filters('app-shortcode-confirmation-email_field_id', 'appointments-field-customer_email')) .'"><span>'. $email . ($appointments->options["ask_email"] ? ' ' : '') . '</span></label><input type="text" autocomplete="off" name="'. esc_attr(apply_filters('app-shortcode-confirmation-email_field_id', 'appointments-field-customer_email')) . '" class="appointments-email-field-entry" id="' . esc_attr(apply_filters('app-shortcode-confirmation-email_field_id', 'appointments-field-customer_email')) . '" />';
		$ret .= '</div>';
		
		$ret .= '<div class="appointments-phone-field" style="display:none">';
		$ret .= '<label for="' . esc_attr(apply_filters('app-shortcode-confirmation-email_field_id', 'appointments-field-customer_phone')) .'"><span>'. $phone . ($appointments->options["ask_phone"] ? ' *' : '') . '</span></label><input type="text" autocomplete="off" name="'. esc_attr(apply_filters('app-shortcode-confirmation-phone_field_id', 'appointments-field-customer_phone')) . '" class="appointments-phone-field-entry" id="' . esc_attr(apply_filters('app-shortcode-confirmation-phone_field_id', 'appointments-field-customer_phone')) . '" />';
		$ret .= '</div>';
		
		$ret .= '<div class="appointments-address-field" style="display:none">';
		$ret .= '<label for="' . esc_attr(apply_filters('app-shortcode-confirmation-address_field_id', 'appointments-field-customer_address')) .'"><span>'. $address . ($appointments->options["ask_address"] ? ' *' : '') . '</span></label><input autocomplete="off" type="text" name="'. esc_attr(apply_filters('app-shortcode-confirmation-address_field_id', 'appointments-field-customer_address')) . '" class="appointments-address-field-entry" id="' . esc_attr(apply_filters('app-shortcode-confirmation-address_field_id', 'appointments-field-customer_address')) . '" />';
		$ret .= '</div>';
		
		$ret .= '<div class="appointments-city-field" style="display:none">';
		$ret .= '<label><span>'. $city . ($appointments->options["ask_city"] ? ' *' : '') . '</span><input autocomplete="off" type="text" class="appointments-city-field-entry" id="' . esc_attr(apply_filters('app-shortcode-confirmation-city_field_id', 'appointments-field-customer_city')) . '" /></label>';
		$ret .= '</div>';
		$ret .= '<div class="appointments-note-field" style="display:none">';
		$ret .= '<label><span>'. $note . ($appointments->options["ask_note"] ? ' *' : '') . '</span><input type="text" autocomplete="off" class="appointments-note-field-entry" id="' . esc_attr(apply_filters('app-shortcode-confirmation-note_field_id', 'appointments-field-customer_note')) . '" /></label>';
		$ret .= '</div>';
		...
		$ret .= '<input type="button" class="button-primary appointments-confirmation-button" value="'.$button_text.'" />';
		$ret .= '<input type="button" class="button-cancel appointments-confirmation-cancel-button" value="'.__('Cancel', 'appointments').'" />';
		$ret .= '</div>';
		$ret .= '</fieldset></form></div>';
		...
		if ( is_user_logged_in() || 'yes' != $appointments->options["login_required"] ) {
			$script .= '$(".appointments-list table td.free, .app_timetable div.free").not(".app_monthly_schedule_wrapper table td.free").click(function(){$(".app_timetable .active").removeClass("active");$(this).addClass("active");';
		...
		$script .= 'if (!$("#appointmentForm").valid()) { return; }';
		...
		$script .= '$.post(_appointments_data.ajax_url, post_data, function(response) {
						$(".wait_img").remove();
						if ( response && response.error ) {
							jQuery("#problem p.info").html(response.error);
							jQuery("#problem").modal();
						}
							...
							jQuery("p.info").html("<strong>'.esc_js($confirm_text).'</strong>");
							jQuery("p.service").html(jQuery(".appointments-confirmation-service label").html());
							jQuery("p.datetime").html(jQuery(".appointments-confirmation-start label").html());
							jQuery("p.length").html(jQuery(".appointments-confirmation-end label").html());
							jQuery("p.name").html("<span>Patient name: </span>" + app_name);
							jQuery("p.email").html("<span>Patient email: </span>" + app_email);
							jQuery("p.phone").html("<span>Patient phone: </span>" + app_phone);
							jQuery("p.address").html("<span>Patient address: </span>" + app_address);
							jQuery("p.dob").html(jQuery(this).data("dateofbirth"));
							jQuery("p.xray").html(jQuery(this).data("xray"));
							jQuery("p.notes").html(jQuery(this).data("dateofbirth"));
							
							jQuery("#confirmed").modal();
														
							jQuery(".print").click(function() {
								
								window.open("print-form/?output=pdf&appointment_id=" + response.app_id, "Manawatu Ultrasound", "");
						...
						jQuery("#problem p.info").html(response.error);
							jQuery("#problem p.info").html("'.esc_js(__('A connection problem occurred. Please try again.','appointments')).'");
							jQuery("#problem").modal();
						}
...
class App_Shortcode_GetDate extends App_Shortcode {
	public function __construct () {
		$this->_defaults = array(
			'value' => array(
				'value' => 0,
				'help' => __('Value to .', 'appointments'),
				'example' => '1',
			)
		);
	}
	...
	public function get_usage_info () {
		return __('Displays appointment form data.', 'appointments');
	}
...
class App_Shortcode_GetAppointment extends App_Shortcode {
	public function __construct () {
		$this->_defaults = array(
			'appointment_id' => array(
				'value' => 0,
				'help' => __('Appointment Id to view.', 'appointments'),
				'example' => '1',
			),
			'column_val' => array(
				'value' => 'name',
				'help' => __('Field to display.', 'appointments'),
				'example' => 'name',
			)
		);
	}
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		$_id = (array_key_exists("appointment_id", $_GET) ? encrypt_decrypt("decrypt", $_GET["appointment_id"]) : $appointment_id);

		$fields = $wpdb->get_results("SELECT column_name FROM information_schema.columns WHERE table_name='" .$appointments->app_table ."' AND COLUMN_NAME = '" . $column_val . "'", ARRAY_A); 
				
		$field_exists = (sizeof($fields) > 0 ? true : false);
							
		if ( $_id > 0 && $field_exists ) {
		
			$app = $wpdb->get_row( "SELECT at." . $column_val . " FROM " . $appointments->app_table . " at JOIN " .
				   $appointments->services_table . " st ON st.id = at.service WHERE at.ID=".$_id." " );
			
			$ret = stripslashes( $app->$column_val );
		} else {
			$fields = !empty($appointments->options['additional_fields']) ?$appointments->options['additional_fields'] : array();
			if (empty($fields)) return $ret;
			
			$app_meta = _get_appointment_meta($_id);
			
			foreach ($fields as $field) {
				$label = esc_html($field['label']);
				$name = _to_clean_name($label);
...
class App_Shortcode_GetAppointmentDate extends App_Shortcode {
	public function __construct () {
		$this->_defaults = array(
			'appointment_id' => array(
				'value' => 0,
			...
			'format' => array(
				'value' => '%W %M %Y',
				'help' => __('Date format mysql.', 'appointments'),
				'example' => '%W %M %Y',
			)
		);
	}
	...
	public function get_usage_info () {
		return __('Displays appointment date.', 'appointments');
	}
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		if ( $_id > 0) {
			$app = $wpdb->get_row( "SELECT DATE_FORMAT(start, '" .$format. "') date FROM " .$appointments->app_table. " WHERE ID=".$_id." " );
			$ret = stripslashes( $app->date );
		} 
		return $ret;
	}
...
class App_Shortcode_GetAppointmentTime extends App_Shortcode {
	public function __construct () {
		$this->_defaults = array(
			'appointment_id' => array(
				'value' => 0,
			...
			'format' => array(
				'value' => '%l:%i %p',
				'help' => __('Time format.', 'appointments'),
				'example' => '%l:%i %p',
			)
		);
	}
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		if ( $_id > 0) {
			$app = $wpdb->get_row( "SELECT DATE_FORMAT(start, '" .$format. "') time FROM " .$appointments->app_table. " WHERE ID=".$_id." " );
			$ret = stripslashes( $app->time );
		} 
		return $ret;
	}
...
class App_Shortcode_GetService extends App_Shortcode {
	public function __construct () {
		$this->_defaults = array(
			'appointment_id' => array(
				'value' => 0,
	...
	public function get_usage_info () {
		return __('Displays service form data.', 'appointments');
	}
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		if ( $_id > 0 && $field_exists ) {
			...
			$app = $wpdb->get_row( "SELECT st." . $column_val . " FROM " . $appointments->app_table . " at JOIN " .
				   $appointments->services_table . " st ON st.id = at.service WHERE at.ID=".$_id." " );
			
			$ret = stripslashes( $app->$column_val );
		} else {
			$app = $wpdb->get_row( "SELECT st.page FROM " . $appointments->app_table . " at JOIN " .
				   $appointments->services_table . " st ON st.id = at.service WHERE at.ID=".$_id." " );
			
			$page = stripslashes( $app->page );
			$srv = $appointments->get_excerpt( $page, 0, '', 0);
			$ret = stripslashes( $srv );
		} 
...
function display_inline_data($appointments, $appointment_id) {
	$form = '';
	$fields = !empty($appointments->options['additional_fields']) ?$appointments->options['additional_fields'] : array();
	if (empty($fields)) return $form;

	$app_meta = _get_appointment_meta($appointment_id);
	...
	foreach ($fields as $field) {
		...
		$form .= '<div class="title">' . $label . ' ' . $value . '</div>';
	}
	return $form;
}

function _get_appointment_meta ($appointment_id) {
...
function pre_process_shortcode() {
 if (!is_singular()) return;
  ...
  if (!empty($post->post_content)) {
    ...
    if (!empty($matches[2]) && in_array('app_login',$matches[2]) && !is_user_logged_in()) {
...
function app_core_shortcodes_register ($shortcodes) {
	...
	$shortcodes['app_get_appointment'] = 'App_Shortcode_GetAppointment';
	$shortcodes['app_get_appointment_date'] = 'App_Shortcode_GetAppointmentDate';
	$shortcodes['app_get_appointment_time'] = 'App_Shortcode_GetAppointmentTime';
	$shortcodes['app_get_service'] = 'App_Shortcode_GetService';
